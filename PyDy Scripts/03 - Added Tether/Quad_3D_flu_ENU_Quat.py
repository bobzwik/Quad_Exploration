...
from sympy import symbols, Matrix, sign
...
W = ReferenceFrame('W')  # Wind reference frame
...
# Wind
# ---------------------------
W.orient(N, 'Body', [qW1, qW2, 0], 'ZYX')
# W.orient(N, 'Quaternion', [qW0, qW1, qW2, qW3])
Wo.set_vel(N, velW*W.x)
airVel = Bcm.vel(N) - Wo.vel(N)
...
dragX = (Bcm, -Cd*sign(dot(airVel, N.x))*(dot(airVel, N.x)**2)*N.x)
dragY = (Bcm, -Cd*sign(dot(airVel, N.y))*(dot(airVel, N.y)**2)*N.y)
dragZ = (Bcm, -Cd*sign(dot(airVel, N.z))*(dot(airVel, N.z)**2)*N.z)
