...
class PotField:
    ...
    def __init__(self):
        ...
        self.withinRange = np.zeros(self.num_points)
        self.withinField = np.zeros(self.num_points)

        self.force = np.zeros(3)
        self.vel   = np.zeros(3)
    
    def isWithinRange(self, quad):
        self.withinRange = (abs(quad.pos[0]-self.pointcloud[:,0]) <= 2.5) & (abs(quad.pos[1]-self.pointcloud[:,1]) <= 2.5) & (abs(quad.pos[2]-self.pointcloud[:,2]) <= 2.5)
